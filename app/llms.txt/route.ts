...
export function GET() {
  ...
  const lines: string[] = [
    ...
    '',
    '## Activité',
    `${businessName} intervient sur les pannes de chauffage et de production d'eau chaude à ${city} et dans les communes de l'agglomération : chaudière à gaz ou au fioul à l'arrêt, pompe à chaleur en défaut, radiateur froid, purge et désembouage du circuit de chauffage, fuite sur une chaudière ou sur le circuit de chauffage, ballon d'eau chaude et cumulus, entretien annuel, ainsi que sur la climatisation en mode rafraîchissement et son entretien. Le périmètre est le dépannage et l'entretien d'appareils existants. Nous ne faisons pas d'installation neuve, pas de poêle à bois ou à granulés, et rien qui relève des canalisations d'évacuation. Une climatisation réversible qui ne chauffe plus en hiver relève de la page pompe à chaleur, une climatisation qui ne refroidit plus en été relève de la page climatisation. Le mot désembouage désigne ici exclusivement le nettoyage du circuit de chauffage (radiateurs et tuyauterie de chauffage), jamais une évacuation d'eaux usées. Le tarif de la prestation est annoncé avant le début de l'intervention.`,
    '',
    '## Méthodes',
    ...methods.map((m) => `- ${m}`),
    '',
