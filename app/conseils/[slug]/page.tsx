...
export default function ArticlePage({ params }: { params: { slug: string } }) {
  ...
  return (
    <>
      ...
      <section className="grain relative overflow-hidden bg-gradient-to-b from-fonte-950 via-fonte-900 to-fonte-950 py-16 lg:py-20">
        ...
        <div className="relative mx-auto max-w-3xl px-6 lg:px-10">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-craie-400">
            ...
            <span className="h-1 w-1 rounded-full bg-craie-600" aria-hidden="true" />
      ...
      <article className="bg-craie-50 py-16 lg:py-20">
        <div className="mx-auto max-w-3xl px-6 lg:px-10">
          ...
          {linked.length > 0 && (
            <AnimatedSection className="mt-14 rounded-bloc border border-flamme-600/20 bg-flamme-600/5 p-8">
                ...
                {linked.map((s) => (
                  <li key={s.slug}>
                    <Link
                      ...
                      className="group flex items-center gap-3 rounded-2xl border border-craie-200 bg-white p-4 transition-all duration-300 hover:-translate-y-0.5 hover:border-flamme-400/40"
                    >
                      <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-flamme-600/10 text-flamme-600">
                        <ServiceIcon icon={s.icon} className="h-5 w-5" />
                      </span>
                      <span className="text-sm font-medium text-fonte-900 group-hover:text-flamme-700">
                        {s.navTitle}
                      </span>
                    </Link>
                  </li>
      ...
      {related.length > 0 && (
        <section className="border-t border-craie-200 bg-craie-100 py-16 lg:py-20" aria-labelledby="a-lire-aussi">
              ...
              {related.map((a, idx) => (
                <AnimatedSection key={a.slug} delay={(idx % 3) * 0.07} as="li">
                  ...
                  >
                    ...
                    <div className="p-6">
                      <span className="text-xs font-semibold uppercase tracking-wider text-braise-600">
                        {a.category}
                      </span>
                      <h3 className="mt-2 font-display text-lg font-medium leading-snug text-fonte-950 group-hover:text-flamme-700">
                        {a.title}
                      </h3>
                    </div>
                  </Link>
